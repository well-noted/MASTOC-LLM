...
def estimate_cost(conditions, ticks, model):
    ...
    for cond in conditions:
        calls = (agent_calls.get(cond, 0) + inst_calls.get(cond, 0)) * ticks
        cost  = calls * in_toks / 1e6 * in_price + calls * out_toks / 1e6 * out_price
        total += cost
        print(f"{cond:<15} {calls:>8.0f} ${cost:>10.3f}")
    print("-" * 40)
    print(f"{'TOTAL':<15} {'':<8} ${total:>10.3f}")
...
def main():
    ...
    llm_cfg = cfg.get("llm", {})
    parser = argparse.ArgumentParser(description="MASTOC-LLM experiment runner")
    parser.add_argument("--mode", required=True, choices=["run", "analyse", "xml", "cost"])
    parser.add_argument("--netlogo-path", default="C:/Program Files/NetLogo 7.0.4")
    parser.add_argument("--nlogo-file", default="MASTOC-LLM.nlogo")
    parser.add_argument("--conditions", nargs="+", default=ALL_CONDITIONS, choices=ALL_CONDITIONS)
    parser.add_argument("--ticks", type=int, default=sim_cfg.get("ticks", 120))
    parser.add_argument("--backend", default=llm_cfg.get("backend", "anthropic"), choices=["anthropic", "ollama"])
    parser.add_argument("--model", default=llm_cfg.get("anthropic_model", "claude-sonnet-4-6"))
    parser.add_argument("--memory-length", type=int, default=sim_cfg.get("memory_length", 5))
    parser.add_argument("--log-dir", default=cfg.get("logging", {}).get("log_dir", "logs"))
    parser.add_argument("--data-dir", default="Data")
    parser.add_argument("--xml-output", default="behaviourspace.xml")
    parser.add_argument("--no-institutions", action="store_true")
    parser.add_argument("--institution-interval", type=int, default=sim_cfg.get("institution_check_interval", 5))
    args = parser.parse_args()

    if args.mode == "run":
        run_experiments(
            netlogo_path=args.netlogo_path, conditions=args.conditions,
            ticks=args.ticks, backend=args.backend, model=args.model,
            memory_length=args.memory_length, log_dir=args.log_dir,
            nlogo_file=args.nlogo_file, detect_institutions=not args.no_institutions,
            institution_interval=args.institution_interval,
        )
    elif args.mode == "analyse":
        analyse_results(log_dir=args.log_dir, data_dir=args.data_dir)
    elif args.mode == "xml":
        write_behaviourspace_xml(output=args.xml_output)
    elif args.mode == "cost":
        estimate_cost(conditions=args.conditions, ticks=args.ticks, model=args.model)
